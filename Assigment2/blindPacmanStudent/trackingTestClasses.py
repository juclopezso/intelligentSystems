...
class GameScoreTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ghosts = [StaticGhostAgent(i) for i in range(1,self.numGhosts+1)]
...
class DoubleInferenceAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        #pac = VisitAllAgent(inferenceFunction, ghosts, self.seed, elapse=True)
        pac = VisitAllAgent(inferenceFunction, moves, ghosts, grades, self.seed, disp, elapse=self.elapse, observe=self.observe, L2Tolerance=self.L2Tolerance, checkUniform = self.checkUniform)
        if self.inference == "ParticleFilter":
            for pfilter in pac.inferenceModules: pfilter.setNumParticles(5000)
        elif self.inference == "MarginalInference":
            moduleDict['inference'].jointInference.setNumParticles(5000)
...
class VisitAllAgent(bustersAgents.BustersAgent):
    ...
    def getAction(self, gameState):
        ...
        if len(self.parent) == 0: self.parent[ (x,y) ] = (x,y,None)

        for move in gameState.getLegalPacmanActions():
            dx, dy = Actions.directionToVector(move)
            nextx, nexty = int(x + dx), int(y + dy)

            if not gameState.hasWall( nextx, nexty ) and (nextx,nexty) not in self.parent:
                invMove = move
                if move == Directions.NORTH:
                    invMove = Directions.SOUTH
                elif move == Directions.SOUTH:
                    invMove = Directions.NORTH
                elif move == Directions.EAST:
                    invMove = Directions.WEST
                elif move == Directions.WEST:
                    invMove = Directions.EAST

                self.parent[ (nextx,nexty) ] = ( x,y, invMove )
                action = move
                break

        if action == None and (x,y) in self.parent:
            return self.parent[ (x,y) ][2]
        elif (x,y) not in self.parent: print "@UT"
        return action
